...
「工具 → ☁️ 雲端同步」可連到你的 **Firebase / Firestore**，讓多台裝置共用同一份資料
（田區、紀錄、契約、行情校正、常送市場）。貼上 firebaseConfig ＋ 自訂同步碼即可，
支援手動上傳/下載與自動同步。設定步驟與安全規則見 **[docs/雲端同步設定.md](../docs/雲端同步設定.md)**。
不設定則各裝置資料獨立，仍可用「資料備份 / 還原」JSON 手動搬移。
